...
class FuzzyDateField(models.DateField):
    """A field that stores a fuzzy date. See the module doc for more
    information.

    The ``_dont_generate_fields`` argument we support is for south
    introspection and must not be used.
    """

    def __init__(self, *args, **kwargs):
        self._do_generate_fields = not kwargs.pop('_dont_generate_fields', False)
        super(FuzzyDateField, self).__init__(*args, **kwargs)

    def contribute_to_class(self, cls, name):
        if self._do_generate_fields:
            # first, create a hidden "precision" field. It is *crucial* that this
            # field appears *before* the actual date field (i.e. self) in the
            # models _meta.fields - to achieve this, we need to change its
            # creation_counter class variable.
            precision_field = models.IntegerField(
                choices=DATE_PRECISIONS, editable=False,
...
try:
    ...
    # So our job here, besides adding our module to the patterns to make
    # South process us, is mainly to ensure that when a FakeORM object is
    # created by South, the "precision" field we generate in the background
    # is not added twice (since South has detected both fields as separate
    # entities, obviously not knowing about our "ownership").
    #
    # The way to do this is apparently to have an argument to our field
    # class which skips the creationg of those extra fields. The South docs
    # at http://south.aeracode.org/wiki/ExtendingIntrospection ominously
    # talk about an alternative route in which we can tell south to ignore
    # our own extra fields, but I can't think of a way to do this (even if
    # the extra field where a special type in it's own module not registered
    # with South, South would then still complain about it, and neither
    # does there seem to be a way to tell the introspector to ignore a
    # field fullout).
    #
    # That's not all. At this point anyway South doesn't seem to support
    # us telling it's introspector to always call out field with a fixed
    # value for an argument (like dont_generate_fields=True). It insists
    # on reading the value from an argument. So we use a trick: The boolean
    # argument our field takes is stored in negated form. As a result,
    # South will always freeze the exact opposite of what the field's
    # actual attribute value is. Since we expect the value to always be
    # False in production, it will always be True in a South freeze, and
    # thus within South FakeORM, the additional fields will not be created.
    #
    # See also: http://south.aeracode.org/ticket/356
    import re
    modelsinspector.add_introspection_rules(
        rules=[(
                (FuzzyDateField,),
                [],
                {'_dont_generate_fields': ['_do_generate_fields', {}]}
            )],
        patterns = [
