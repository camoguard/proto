...
urlpatterns = patterns('proto.forums.views',
    ...
    url(r'^(?P<forum_slug>[-\w]+)/(?P<thread_pk>\d+)/process-post/$', 'process_post_form', name='process-post'),
    url(r'^(?P<forum_slug>[-\w]+)/(?P<thread_pk>\d+)/$', PostListView.as_view(), name='post-list'),
